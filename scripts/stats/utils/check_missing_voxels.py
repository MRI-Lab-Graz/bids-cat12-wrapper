...
def parse_args():
    ...
    p.add_argument(
        ...
        type=float,
        default=0.05,
        help="Fraction threshold to mark voxel as missing/excluded (default: 0.05)",
    )
    p.add_argument(
        "--fail-if-pct-excluded",
        type=float,
        default=None,
...
def load_spm_image_list(spm_path):
    ...
    if matlab is not None:
        try:
            ...
            cmd = [
                ...
                "-nosplash",
                "-batch",
                f"load('{spm_path}'); for i=1:length(SPM.xY.VY), disp(SPM.xY.VY(i).fname); end; exit;",
            ]
    ...
    raise RuntimeError(
        "Could not extract image filenames from SPM.mat; install scipy or set MATLAB env"
    )
...
def main():
    ...
    if nb is None or np is None:
        print(
            "Required Python packages (nibabel, numpy) are not available in this environment."
        )
    ...
    if gm_mask_path is not None:
        try:
            ...
            if gm_mask_arr.shape != data_shape:
                print(
                    f"Warning: GM mask shape {gm_mask_arr.shape} != image data shape {data_shape}"
                )
                if getattr(args, "require_gm_mask", False):
                    print(
                        "ERROR: require-gm-mask is set but mask shape mismatches image template. Aborting."
                    )
                    return 1
                else:
                    print(
                        "Ignoring GM mask due to shape mismatch and continuing with whole-image calculations"
                    )
                    gm_mask_arr = None
            else:
                print(
                    f"Using GM mask: {gm_mask_path} (restricting calculations to mask voxels)"
                )
        except Exception as e:
            print(f"Warning: failed to load GM mask {gm_mask_path}: {e}")
            if getattr(args, "require_gm_mask", False):
                print(
                    "ERROR: require-gm-mask is set but GM mask could not be loaded. Aborting."
                )
                return 1
            gm_mask_arr = None
        ...
        with open(subj_csv, "w", newline="") as fh:
            ...
            writer.writerow(
                [
                    "subject",
                    "n_images",
                    "n_voxels_missing_any",
                    "pct_voxels_missing_any",
                ]
            )
            for subj, mask in sorted(subj_masks.items()):
                ...
                writer.writerow(
                    [subj, int(subj_counts.get(subj, 0)), n_missing, round(pct, 4)]
                )
    ...
    try:
        ...
        fail_thresh = (
            float(args.fail_if_pct_excluded)
            if hasattr(args, "fail_if_pct_excluded")
            and args.fail_if_pct_excluded is not None
            else None
        )
    except Exception:
    ...
    if fail_thresh is not None:
        if pct_excluded > float(fail_thresh):
            print(
                f"ERROR: pct_voxels_excluded={pct_excluded:.3f}% > fail threshold {fail_thresh}%"
            )
            return 2
