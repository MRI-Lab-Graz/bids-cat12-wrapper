...
def find_participant_column(df):
    for col in df.columns:
        if (
            "participant" in col.lower()
            or "participant_id" in col.lower()
            or col.lower() == "id"
        ):
            return col
    return None
...
def main():
    p = argparse.ArgumentParser(
        description="Merge covariates TSV into participants.tsv"
    )
    p.add_argument("--participants", required=True, help="Path to participants.tsv")
    p.add_argument(
        "--covariates",
        required=True,
        help="Path to covariates TSV (from extract_covariates_from_xml.py)",
    )
    p.add_argument("--out", required=True, help="Output merged participants TSV")
    p.add_argument(
        "--session-col",
        default="session",
        help="Session column name in participants file",
    )
    args = p.parse_args()
    ...
    if "subject" in dfc.columns:
        dfc["subject_norm"] = (
            dfc["subject"].astype(str).str.replace("^sub[-_]", "", regex=True)
        )
    else:
    ...
    dfp["__part_norm"] = (
        dfp[part_col].astype(str).str.replace("^sub[-_]", "", regex=True)
    )

    measures = [
        c
        for c in dfc.columns
        if c not in ("file_path", "subject", "subject_norm", "session")
    ]
    ...
    if "nr_sessions" in dfp.columns:
        ...
        if session_level:
            ...
            if len(rows) == 0:
                print(
                    "Warning: attempted expansion to session-level but found no session rows; falling back to subject-level aggregation"
                )
                session_level = False
            else:
                ...
                dfc["__ses_str"] = (
                    dfc["session"].astype(str) if "session" in dfc.columns else ""
                )

                merged = pd.merge(
                    dfp_exp,
                    dfc[["subject_norm", "__ses_str"] + measures],
                    left_on=["__part_norm", "__ses_str"],
                    right_on=["subject_norm", "__ses_str"],
                    how="left",
                )
                # If merged produced no matched covariates, fall back to subject-level aggregation
                if merged[measures].isna().all().all():
                    print(
                        "Warning: No session-level covariates matched after expansion; falling back to subject-level aggregation"
                    )
                    session_level = False
                else:
                    dfp = merged
        ...
        if not session_level:
            ...
            for subj, row in dfc.groupby("subject_norm"):
                ...
                for m in measures:
                    ...
                    if len(colvals) == 0:
                        ...
                        if len(colvals) > 1:
                            print(
                                f"Warning: multiple values for covariate {m} for subject {subj}; taking the first"
                            )
                        vals[m] = colvals[0]
                sub_vals[subj] = vals
            ...
            for m in measures:
                dfp[m] = dfp["__part_norm"].map(
                    lambda s: sub_vals.get(s, {}).get(m, None)
                )
        ...
        if args.session_col not in dfp.columns:
            print(
                f"ERROR: participants.tsv appears scan-level but session column '{args.session_col}' not found"
            )
            sys.exit(2)
        ...
        if merged[measures].isna().all().all():
            print(
                "Warning: No covariates matched by (participant,session). Trying participant-only merge..."
            )
            merged = pd.merge(
                dfp,
                dfc[["subject_norm"] + measures].drop_duplicates("subject_norm"),
                left_on="__part_norm",
