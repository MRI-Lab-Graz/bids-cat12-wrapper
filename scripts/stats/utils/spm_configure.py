...
def main(argv=None):
    ...
    cfg.add_argument(
        "--save", action="store_true", help="Save detected path to config file"
    )
    cfg.add_argument(
        "--config-file",
        default="spm_config.txt",
        help="Where to save configuration if --save",
    )
    ...
    if not test_spm_python():
        print(
            "spm-python is not installed in this environment. Install with: pip install spm-python",
            file=sys.stderr,
        )
        return 3
    ...
    spm_path = None
    if args.spm_path:
        if os.path.isdir(args.spm_path) and os.path.exists(
            os.path.join(args.spm_path, "spm.m")
        ):
            spm_path = os.path.abspath(args.spm_path)
        else:
            print(
                f"Provided spm path invalid or missing spm.m: {args.spm_path}",
                file=sys.stderr,
            )
            return 4
    else:
    ...
    if not spm_path:
        print(
            "Could not auto-detect an SPM installation. Provide --spm-path or install SPM standalone.",
            file=sys.stderr,
        )
        return 5
    ...
    if not ok:
        print(
            "SPM failed to initialize via spm-python. Ensure MATLAB Runtime is installed and compatible.",
            file=sys.stderr,
        )
        return 6
