...
try:
    ...
    print(
        "ERROR: nibabel not available in this Python. Activate the project venv or install nibabel."
    )
    raise
...
def build_entries_for_d21(
    design_files, session_a="1", session_b="2", use_unsmoothed=False
):
    # build subj_map
    subj_map = {}
...
def main():
    ...
    p.add_argument(
        "--plot-nans",
        action="store_true",
        help="Save a barplot (subject x, number of NaNs in delta y) as PNG",
    )
    p.add_argument(
        "--plot-out",
        default="nan_counts_barplot.png",
        help="Output path for the NaN counts barplot PNG",
    )
    p.add_argument(
        "--nan-csv",
        default=None,
        help="Optional CSV path to write per-subject NaN/mask stats",
    )
    args = p.parse_args()
    ...
    design = find_images_for_subjects(
        parts, args.data_root, session_ids=[str(args.session_a), str(args.session_b)]
    )
    ...
    subj_ids, subj_groups, entries = build_entries_for_d21(
        design.get("files", []),
    ...
    for idx, ent in enumerate(entries[: args.max_subjects]):
        ...
        # report NaN/inf
        print(
            "  img1: min/max/mean/finite/allfinite:",
            np.nanmin(a1),
            np.nanmax(a1),
            np.nanmean(a1),
            np.isfinite(a1).all(),
        ...
        print(
            "  img2: min/max/mean/finite/allfinite:",
            np.nanmin(a2),
            np.nanmax(a2),
            np.nanmean(a2),
            np.isfinite(a2).all(),
        ...
        if "mdata" in locals() and mdata.shape == d.shape:
            ...
            pct_mask_nan = (
                float(n_nan_in_mask) / float(n_mask_vox) if n_mask_vox > 0 else 0.0
            )
    ...
    if os.path.exists(args.mask):
        ...
        if mask_small.shape == stacked.shape[:3]:
            ...
            print(
                "Mask shape does not match image shape; mask shape:",
                mdata.shape,
                "image shape:",
                stacked.shape[:3],
            )
