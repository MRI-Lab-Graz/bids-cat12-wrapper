...
def load_participants_tsv(tsv_path, group_col=None):
    ...
    for row in data_rows:
        ...
        if group_idx is not None and group_idx < len(header):
            try:
                preferred = (
                    ...
                    else entry.get(header[group_idx])
                )
        ...
        entry["subject"] = (
            ...
            or entry.get("participant")
            or entry.get(header[0])
        )
...
def find_images_for_subjects(participants, data_root, session_ids=None):
    ...
    for p in participants:
        ...
        for sid in session_ids:
            ...
            if cand:
                results.append(
                    {"subject": subj, "session": sid, "group": group, "path": cand}
                )

    return {"files": results}


def build_subject_delta_images(
    design, session_a=None, session_b=None, use_unsmoothed=False, group_map=None
):
        ...
        for subj, sessdict in subj_map.items():
            if sa in sessdict and sb in sessdict:
                ...
                if (
                    ...
                    and subj in group_map
                    and group_map.get(subj) is not None
                ):
                ...
                warnings.warn(
                    f"Subject {subj} missing sessions {sa} or {sb} (skipping)"
                )
...
def build_subject_delta_entries_for_type(
    design,
    delta_type="d21",
    use_unsmoothed=False,
    group_map=None,
    session_a=None,
    session_b=None,
):
    ...
    for subj, sessdict in subj_map.items():
        ...
        if delta_type in ("d21", "d32", "d31"):
            ...
            if "1" in sessdict and "2" in sessdict and "3" in sessdict:
                ...
                warnings.warn(
                    f"Subject {subj} missing one of sessions 1/2/3 (skipping)"
                )
...
def run_classification(
    ...
    if classifier_name == "logistic":
        ...
        if class_weight == "balanced":
            warnings.warn(
                "GradientBoostingClassifier does not support class_weight; ignoring --class-weight"
            )
        clf_core = GradientBoostingClassifier(
            n_estimators=100, random_state=random_state
        )
    elif classifier_name == "nb":
        if class_weight == "balanced":
            warnings.warn(
                "GaussianNB does not support class_weight; ignoring --class-weight"
            )
        clf_core = GaussianNB()
    elif classifier_name == "knn":
        if class_weight == "balanced":
            warnings.warn(
                "KNeighborsClassifier does not support class_weight; ignoring --class-weight"
            )
        clf_core = KNeighborsClassifier(n_neighbors=5)
    else:
        if class_weight == "balanced":
            clf_core = LogisticRegression(
                solver="liblinear", penalty="l2", max_iter=1000, class_weight="balanced"
            )
        else:
            clf_core = LogisticRegression(
                solver="liblinear", penalty="l2", max_iter=1000
    ...
    print(
        f"Running permutation test with n_permutations={n_permutations} (this may take some time)"
    )
    perm_scores = []
    ...
    for i in range(n_permutations):
        ...
        if (i + 1) % report_every == 0 or i == n_permutations - 1:
            ...
            print(
                f"  Permutations: {i + 1}/{n_permutations} ({per:.1f}%) elapsed {elapsed:.0f}s eta {eta:.0f}s"
            )
    ...
    class_report = classification_report(
        y_enc, y_pred_cv, target_names=le.classes_, digits=3
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="VBM ML interaction prototype (nilearn+sklearn)"
    )
    parser.add_argument("--design-json", required=False, help="Path to design.json")
    parser.add_argument(
        "--mask",
        default="templates/brainmask_GMtight.nii",
        help="Path to brain mask NIfTI",
    )
    parser.add_argument("--output", required=True, help="Output directory")
    parser.add_argument(
        "--n-permutations", type=int, default=1000, help="Permutation tests"
    )
    ...
    parser.add_argument(
        "--participants-tsv", help="BIDS participants.tsv file (optional)"
    )
    parser.add_argument(
        "--data-root",
        help="Path to subject data root (used with participants.tsv). e.g. /Volumes/Thunder/129_PK01/cat12/data/cat12",
    )
    parser.add_argument(
        "--classifier",
        choices=["logistic", "svc", "linear_svc", "rbf_svc", "rf", "gb", "nb", "knn"],
        default="logistic",
        help="Classifier to use.",
    )
    parser.add_argument(
        "--group-col",
        help="Column name or 1-based index for group in participants.tsv.",
    )
    parser.add_argument(
        "--session1-tokens",
        nargs="+",
        default=["ses-1", "ses1", "ses-01"],
        help="Tokens to identify session 1 files.",
    )
    parser.add_argument(
        "--session2-tokens",
        nargs="+",
        default=["ses-2", "ses2", "ses-02"],
        help="Tokens to identify session 2 files.",
    )
    parser.add_argument(
        "--session-a",
        help="Session id (as in design.json) to use as first timepoint (e.g., 1)",
    )
    parser.add_argument(
        "--session-b",
        help="Session id (as in design.json) to use as second timepoint (e.g., 2)",
    )
    parser.add_argument(
        "--use-unsmoothed",
        action="store_true",
        help="Prefer unsmoothed images (mwp1*) when available by attempting filename substitution",
    )
    parser.add_argument(
        "--n-jobs",
        type=int,
        default=1,
        help="Number of parallel jobs to use for cross-validation (passed to cross_val_score)",
    )
    parser.add_argument(
        "--class-weight",
        choices=["balanced", "none"],
        default="none",
        help='If "balanced", pass class_weight="balanced" to classifiers that support it',
    )
    parser.add_argument(
        "--delta-type",
        choices=["d21", "d32", "d31", "dsec", "concat"],
        default="d21",
        help="Which per-subject delta to compute: d21=(T2-T1), d32=(T3-T2), d31=(T3-T1), dsec=(T3-2*T2+T1), concat=(D21 and D32 concatenated features)",
    )
    parser.add_argument(
        "--merge-interventions",
        action="store_true",
        help='Merge any group containing "intervention" into a single "intervention" group',
    )
    parser.add_argument(
        "--min-finite-prop",
        type=float,
        default=0.9,
        help="Minimum proportion (0-1) of subjects with finite voxel values required to keep a voxel (default 0.9)",
    )
    ...
    if args.design_json and os.path.isfile(args.design_json):
        ...
        if args.participants_tsv and args.data_root:
            participants = load_participants_tsv(
                args.participants_tsv, group_col=args.group_col
            )
            ...
            if args.session_a is not None and args.session_b is not None:
                ...
                if getattr(args, "delta_type", None) in ("d21", None):
                    ...
                    session_ids = ["1", "2"]
            design = find_images_for_subjects(
                participants, args.data_root, session_ids=session_ids
            )
            print(
                f"Built design from participants TSV: {args.participants_tsv} (group_col={args.group_col})"
            )
        else:
            ...
            if os.path.isfile(cwd_design):
                ...
                raise SystemExit(
                    "No design.json found and participants/data-root not provided; aborting"
                )
    ...
    if getattr(args, "participants_tsv", None) and not os.path.isfile(
    ...
    if getattr(args, "participants_tsv", None) and os.path.isfile(
        args.participants_tsv
    ):
        participants = load_participants_tsv(
            args.participants_tsv, group_col=args.group_col
        ...
        for e in design.get("files", []):
            ...
            if sub in subj_group_map and subj_group_map[sub] is not None:
                if e.get("group") != subj_group_map[sub]:
                    ...
                    replaced += 1
        print(
            f"Overrode group labels from participants.tsv for {replaced} design entries"
        )
    ...
    subj_ids, subj_groups, subj_entries = build_subject_delta_entries_for_type(
        design,
        delta_type=args.delta_type,
        use_unsmoothed=args.use_unsmoothed,
        group_map=subj_group_map,
    ...
    missing_idx = [
        i for i, g in enumerate(subj_groups) if g is None or str(g).strip() == ""
    ]
    if missing_idx:
        warnings.warn(
            f"Found {len(missing_idx)} subjects with missing group labels; these subjects will be skipped"
        )
    ...
    if n_groups < 2:
        raise SystemExit(
            f"Found {n_groups} unique group(s) after processing; need at least 2 groups to run classification; groups found: {list(grp_counts.keys())}"
        )
    ...
    if min_class_count < cv_folds_requested:
        warnings.warn(
            f"Minimum class count ({min_class_count}) is smaller than requested cv_folds ({cv_folds_requested}); reducing cv_folds to {min_class_count}"
        )
    ...
    arrays, imgs, is_concat = compute_delta_arrays(
        subj_entries, delta_type=args.delta_type
    )
    ...
    try:
        ...
        n_finite_total = 0
    print(
        f"Voxel filtering: total voxels={X_all.shape[1]}, finite_in_at_least_1={n_finite_total}, finite_threshold_count={min_count}, voxels_passing_finite_count={int(np.sum(finite_mask))}, voxels_passing_variance={int(np.sum(valid_var))}, voxels_final={n_valid}"
    )
    if n_valid == 0:
        raise RuntimeError(
            "No valid voxels found after filtering constant/NaN columns (try lowering --min-finite-prop or inspect input images for NaNs)"
        )
    ...
    coefs_reduced, summary = run_classification(
        X,
        subj_groups,
        args.mask,
        outdir,
    ...
    if is_concat:
        ...
        if len(coefs_reduced) != 2 * m:
            ...
            warnings.warn(
                "Unexpected coefficient length for concat mode; saving reduced coefficients as text"
            )
        ...
        if len(coefs_reduced) != int(np.sum(voxel_valid_mask)):
            ...
            warnings.warn(
                "Coefficient length does not match valid voxel count; saving reduced coefficients as text"
            )
            with open(os.path.join(outdir, "coefs_reduced.txt"), "w") as f:
                np.savetxt(f, np.atleast_1d(coefs_reduced))
        else:
            ...
            display = plot_stat_map(
                str(coef_path),
                title="ML coefficient map (delta maps)",
                display_mode="z",
                cut_coords=6,
            )
