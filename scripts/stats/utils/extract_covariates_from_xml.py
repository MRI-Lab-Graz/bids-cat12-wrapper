...
def load_spm_file_list(spm_mat):
    ...
    if P is None:
        print(
            "ERROR: Could not find xY.P inside SPM.mat. The file may be saved in an unusual format."
        )
...
def main():
    p = argparse.ArgumentParser(
        description="Extract covariates from CAT12 XMLs in SPM.mat or participants order"
    )
    p.add_argument(
        "--spm",
        required=False,
        help="Path to SPM.mat (preferred for exact SPM ordering)",
    )
    p.add_argument(
        "--participants",
        required=False,
        help="Path to participants.tsv to determine ordering when SPM.mat is not available",
    )
    p.add_argument(
        "--cat12",
        required=True,
        help="Path to CAT12 directory (contains sub-*/report/*.xml)",
    )
    p.add_argument("--out", required=True, help="Output TSV file to write (ordered)")
    p.add_argument(
        "--measures",
        default="vol_TIV,SQR,ICR,SurfaceEulerNumber",
    ...
    if spm_mat:
        ...
        for f in files:
            ...
            if xml is None:
                print(
                    f"Warning: XML report not found for subject {subj} (session {ses}) under {cat12_dir}"
                )
                vals = {m: "" for m in measures}
            else:
                vals = extract_measures_from_xml(xml, measures)

            row = [f, subj, ses if ses is not None else ""] + [
                vals.get(m, "") for m in measures
            ]
            rows.append(row)
    elif participants:
        ...
        if "session" in dfp.columns and dfp["session"].notna().any():
            ...
            for _, prow in dfp.iterrows():
                ...
                if xml is None:
                    print(
                        f"Warning: XML report not found for subject {subj} session {ses}"
                    )
                    vals = {m: "" for m in measures}
                else:
                    vals = extract_measures_from_xml(xml, measures)
    ...
    print(
        "Note: order matches file order in SPM.mat. Use this TSV as scan-level participants.tsv or pass --covariates accordingly."
    )
