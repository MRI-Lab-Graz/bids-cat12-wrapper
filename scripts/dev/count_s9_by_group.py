...
def count_subjects_with_data():
    ...
    for group in groups:
        ...
        for _, row in group_df.iterrows():
            ...
            search_pattern = os.path.join(data_dir, subject_id, 'mri', f's9mwp1*{subject_id}_ses-1_acq-mprage_T1w.nii')
        ...
        print(f"{group:<10} | {n_total:<10} | {n_with_data:<15} | {n_total - n_with_data:<10}")
        
        total_subjects += n_total
        total_with_data += n_with_data

    print("-" * 55)
    print(f"{'Total':<10} | {total_subjects:<10} | {total_with_data:<15} | {total_subjects - total_with_data:<10}")
