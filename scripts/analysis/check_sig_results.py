...
def get_label_at_coordinate(coord, atlas_img, atlas_data, labels):
    ...
    # Check bounds
    if (
        0 <= voxel_coord[0] < atlas_data.shape[0]
        and 0 <= voxel_coord[1] < atlas_data.shape[1]
        and 0 <= voxel_coord[2] < atlas_data.shape[2]
    ):
...
def check_and_sort_significant_results(
    directory, threshold=1.3, atlas_names=None, min_cluster_size=0
):
        ...
        for name in atlas_names:
            ...
            if os.path.exists(atlas_path):
                try:
                    ...
                    atlases.append(
                        {"name": name, "img": img, "data": data, "labels": labels}
                    )
    ...
    print(
        f"Sorting significant results (> {threshold} to 'pos', < -{threshold} to 'neg')"
    )
    ...
    for file_path in files:
        try:
                    ...
                    clusters.append(
                        {
                            ...
                            "coord": peak_world,
                            "regions": [
                                get_label_at_coordinate(
                                    peak_world, a["img"], a["data"], a["labels"]
                                )
                                for a in atlases
                            ],
                        }
            ...
            if clusters:
                ...
                for c in clusters:
                    coord_str = (
                        f"{c['coord'][0]:.1f}, {c['coord'][1]:.1f}, {c['coord'][2]:.1f}"
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check and sort significant results based on a threshold."
    )
    parser.add_argument(
        "-t",
        "--threshold",
        type=float,
        default=1.3,
        help="Threshold for significance (default: 1.3)",
    )
    parser.add_argument(
        "-d",
        "--directory",
        type=str,
        default="./results/vbm/2groups_tiv_age_sex",
        help="Directory to check (default: ./results/vbm/2groups_tiv_age_sex)",
    )
    parser.add_argument(
        "--atlas",
        type=str,
        default="aal3",
        help="Comma-separated list of atlas names (e.g., 'aal3, cobra'). Files must be in templates/ folder.",
    )
    parser.add_argument(
        "-k",
        "--min-cluster-size",
        type=int,
        default=0,
        help="Minimum cluster size in voxels (default: 0)",
    )
    ...
    if args.atlas.lower() == "all":
        ...
        atlas_names = (
            [name.strip() for name in args.atlas.split(",")] if args.atlas else []
        )

    # Check if directory exists
    if os.path.exists(args.directory):
        check_and_sort_significant_results(
            args.directory,
