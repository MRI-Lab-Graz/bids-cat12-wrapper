...
class SubjectProcessor:
    ...
    def process_subject(
        self, subject_id: str, t1w_files: List[str], sessions: List[str]
    ) -> Dict:
        """
        ...
        try:
            ...
            script_path = self.script_generator.generate_longitudinal_script(
                subject=subject_id, t1w_files=t1w_files, output_dir=subject_output_dir
            )
                ...
                if self._validate_outputs(subject_output_dir):
                    ...
                    if self.config["cat12"]["quality_check"]:
                        results["quality_metrics"] = (
                            self.quality_checker.check_subject_outputs(
                                subject_output_dir
                            )
                        )
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process individual subject with CAT12"
    )
    ...
    if results["success"]:
        ...
        print(
            f"Subject {args.subject_id} processing failed: {results['error_message']}"
        )
        sys.exit(1)
