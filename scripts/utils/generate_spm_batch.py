...
def generate_cells_code(design, subject_map):
    ...
    for g_idx, group in enumerate(groups, 1):
        for s_idx, session in enumerate(sessions, 1):
            ...
            subj_ids = [
                finfo["subject"] for finfo in design["files"] if finfo["path"] in files
            ]
...
def generate_covariates_code(design):
    ...
    for cov_idx, (cov_name, cov_values) in enumerate(design["covariates"].items(), 1):
        ...
        disp_name = (
            unicodedata.normalize("NFKD", str(cov_name))
            .encode("ascii", "ignore")
            .decode("ascii")
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--design-file", required=True, help="Path to design.json file")
    parser.add_argument(
        "--output-dir", required=True, help="Output directory for SPM results"
    )
    parser.add_argument("--modality", required=True, help="Analysis modality")
    parser.add_argument("--output", required=True, help="Output path for batch .m file")
    parser.add_argument(
        "--mask-file",
        default=None,
