...
def build_cmd(global_cfg, job_cfg, outdir_job):
    ...
    for bool_key, flagname in [
        ("use_unsmoothed", "--use-unsmoothed"),
        ("merge_interventions", "--merge-interventions"),
    ]:
...
def run_jobs(
    ...
    phase1_perms=50,
    phase2_perms=5000,
    promote_pvalue_threshold=0.05,
    promote_pvalue_margin=0.02,
    promote_top_k=0,
):
        ...
        for job in job_list:
            ...
            try:
                ...
                if proc.returncode != 0:
                    print(
                        f"Job {name} failed (returncode {proc.returncode}); see {log_path}"
                    )
                ...
                print(
                    "  Suggestion: ensure `python_executable` in the config is an absolute path or exists relative to the project root."
                )
    ...
    if two_phase:
        ...
        if promote_top_k and promote_top_k > 0:
            ...
            scored_sorted = sorted(
                scored,
                key=lambda x: float(x.get("cv_accuracy_mean", 0.0)),
                reverse=True,
            )
        ...
        if promote_candidates:
            print(
                f"Promoting {len(promote_candidates)} candidates to phase2 (n_permutations={phase2_perms})"
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--config",
        required=True,
        help="Path to JSON config (see utils/ml_batch_config_example.json)",
    )
    ...
    p.add_argument(
        ...
        dest="resume",
        action="store_false",
        help="Do not skip jobs that have existing ml_summary.json",
    )
    p.add_argument(
        "--dry-run", action="store_true", help="Show commands but do not execute"
    )
    p.add_argument(
        "--two-phase",
        action="store_true",
        help="Run a two-phase workflow: low-permutation sweep then promote promising jobs to high-permutation re-runs",
    )
    p.add_argument(
        "--phase1-perms",
        type=int,
        default=50,
        help="Number of permutations for phase1 (coarse scan)",
    )
    p.add_argument(
        "--phase2-perms",
        type=int,
        default=5000,
        help="Number of permutations for phase2 (confirmation)",
    )
    p.add_argument(
        "--promote-pvalue-threshold",
        type=float,
        default=0.05,
        help="P-value threshold for promotion",
    )
    p.add_argument(
        "--promote-pvalue-margin",
        type=float,
        default=0.02,
        help="Additional margin above threshold to promote (useful for borderline candidates)",
    )
    p.add_argument(
        "--promote-top-k",
        type=int,
        default=0,
        help="Additionally promote top-K jobs by accuracy regardless of p-value (0 = disabled)",
    )
    args = p.parse_args()
