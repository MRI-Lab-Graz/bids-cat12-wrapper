...
def validate_design(json_file):
    ...
    for cov_name, cov_values in covariates.items():
        if len(cov_values) != len(files):
            print(
                f"❌ ERROR: Covariate '{cov_name}' has {len(cov_values)} values, but there are {len(files)} files."
            )
            issues_found = True
        else:
    ...
    if len(prefix_counts) > 1:
        ...
        print(
            "  This means you are mixing files with different smoothing kernels (e.g., s6, s9, or none)."
        )
        ...
        print(
            "⚠️ Warning: No prefixes detected (filenames might not start with standard CAT12 prefixes)."
        )
    ...
    if len(set(session_counts)) > 1:
        ...
        print(
            f"✓ Balanced design: All {len(subjects)} subjects have {session_counts[0]} sessions."
        )
