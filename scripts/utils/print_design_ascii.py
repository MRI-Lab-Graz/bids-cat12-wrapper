...
parser = argparse.ArgumentParser(
    description="Print ASCII design matrix preview from design.json"
)
parser.add_argument(
    "design",
    nargs="?",
    help="Path to design.json (default: ./design.json)",
    default="design.json",
)
parser.add_argument(
    "--output",
    "-o",
    help="Output ASCII file path (default: <design_dir>/design_ascii.txt)",
)
parser.add_argument(
    "--rows",
    "-n",
    type=int,
    default=20,
    help="Number of rows to include in terminal preview",
)
parser.add_argument(
    "--width", "-w", type=int, default=80, help="Terminal width for ASCII matrix"
)
...
if not p.exists():
    ...
    if candidates:
        ...
        candidates_sorted = sorted(
            candidates, key=lambda x: ("/results/vbm/" in str(x), x.stat().st_mtime)
        )
...
header = (
    "   subject      s "
    + "".join([f"{i % 10}" for i in range(len(unique_cells[:cell_area]))])
    + "  "
    + "".join([c[0] for c in list(cov_lists.keys())[:cov_area]])
)
lines.append(header)
...
if cov_lists:
    ...
    for cname, vals in cov_lists.items():
        ...
        if nums:
            lines.append(
                f" {cname}: n={len(nums)}, mean={statistics.mean(nums):.3f}, sd={statistics.pstdev(nums):.3f}"
            )
        else:
