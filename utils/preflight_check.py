...
def gather_expected_sessions(participants_file: str, session_col: str) -> Dict[str, List[str]]:
    ...
    if 'nr_sessions' in df.columns:
        for _, row in df.iterrows():
            ...
            if nr_sessions < 1:
                continue
            expected.setdefault(subject, []).extend([str(i) for i in range(1, nr_sessions + 1)])
    else:
        if session_col not in df.columns:
            raise ValueError(f"participants.tsv is scan-level but lacks session column '{session_col}'")
        for _, row in df.iterrows():
            subject = row['participant_id']
...
def check_xml_reports(cat12_dir: str, participants_file: str, session_col: str) -> bool:
    ...
    try:
        import parse_participants as parser_module
    except Exception as exc:
        print(f"ERROR: Unable to import parse_participants helper: {exc}")
        return False
    ...
    get_subject_dirs = getattr(parser_module, '_subject_data_dirs', None)
    ...
    for subject, sessions in expected.items():
        if not subjects_with_data.get(subject):
            continue
        for session in sessions:
            xml_path = parser_module.find_xml_for_subject(cat12_dir, subject, session)
            if not xml_path:
    ...
    if missing:
        ...
        print("Please rerun CAT12 preprocessing or add the XML files before running the pipeline.")
        return False
...
def check_tiv_presence(cat12_dir: str, participants_file: str, session_col: str) -> bool:
    ...
    try:
        ...
        print(f"ERROR: Unable to import parse_participants helper for TIV check: {exc}")
        return False
    ...
    if tiv_col is not None:
        ...
        if nonnull > 0:
            print(f"✓ TIV column '{tiv_col}' found in participants.tsv ({nonnull} non-empty entries)")
            return True
        else:
            print(f"Note: TIV column '{tiv_col}' exists but contains only missing values; will try XML extraction")
    ...
    if resolve_alias is None:
        def _resolve(x):
            return 'vol_TIV' if x.lower() == 'tiv' else x
    else:
    ...
    if missing_tiv:
        print("ERROR: TIV (vol_TIV) not found for the following subject/sessions (only subjects with CAT12 data are checked):")
        for subj, ses in missing_tiv[:20]:
            print(f"  - {subj} ses-{ses}")
        if len(missing_tiv) > 20:
            print(f"  ... and {len(missing_tiv)-20} more")
        print("Please add TIV to your participants.tsv or ensure CAT12 XMLs contain 'vol_TIV' entries for these subjects.")
        return False
...
def check_covariates_presence(cat12_dir: str, participants_file: str, session_col: str, covariates: List[str], allow_missing: bool = False) -> bool:
    ...
    try:
        ...
        print(f"ERROR: Unable to import parse_participants helper for covariate check: {exc}")
        return False
    ...
    for cov in covariates:
        ...
        if cov in df.columns and df[cov].notna().sum() > 0:
            print(f"✓ Covariate '{cov}' present in participants.tsv ({df[cov].notna().sum()} non-empty entries)")
            continue
        ...
        if resolve_alias is None:
            def _resolve_local(x):
                return XML_COVARIATE_ALIASES.get(x.lower(), x)
        else:
        ...
        if missing_entries:
            if not allow_missing:
                print(f"ERROR: Covariate '{cov}' missing for the following subject/sessions (only subjects with CAT12 data are checked):")
                ...
                print(f"⚠ Covariate '{cov}' missing for {len(missing_entries)} subject/sessions; continuing because --allow-missing-covariates is set")
        else:
            print(f"✓ Covariate '{cov}' available via CAT12 XML for all checked subjects")
...
def check_cat12_dir(cat12_dir: str, smoothing: str) -> bool:
    ...
    if not samples:
        print(f"WARNING: No smoothed mwp1r NIfTI files found with pattern {pattern} under {data_dir}")
    ...
    try:
        ...
        print(f"✓ Sample NIfTI OK: {os.path.basename(sample)} shape={data_shape} bytes={os.path.getsize(sample)}")
    except Exception as exc:
...
def find_and_copy_cat12_brainmask(cat12_dir: str) -> str | None:
    candidates: List[str] = []
    candidates.append(os.path.join(cat12_dir, 'templates_MNI152NLin2009cAsym', 'brainmask_GMtight.nii'))
    ...
    if found is None:
        matches = glob.glob(os.path.join(cat12_dir, '**', 'brainmask_GMtight*.nii*'), recursive=True)
        if matches:
            found = matches[0]
...
def check_participants_file(participants_file: str) -> bool:
    ...
    try:
        ...
        if len(lines) < 2:
            print(f"ERROR: Participants file appears empty or only header: {participants_file}")
            return False
...
def main() -> None:
    ...
    parser.add_argument('--cat12-dir', required=True, help='Path to CAT12 data folder or repository root')
    parser.add_argument('--participants', required=True, help='participants.tsv path')
    parser.add_argument('--covariates', default='', help='Comma-separated covariates to check (e.g., "age,sex,tiv")')
    parser.add_argument('--allow-missing-covariates', action='store_true', help='Allow missing covariates (will continue and drop missing ones)')
    ...
    if covs:
        ...
        if not check_covariates_presence(args.cat12_dir, args.participants, args.session_col, covs, allow_missing=args.allow_missing_covariates):
            overall_ok = False
    else:
    ...
    if not os.path.exists(dest):
        ...
        if copied is None or not os.path.exists(dest):
            ...
            print("  - Run this script again with --mask /path/to/brainmask_GMtight.nii")
