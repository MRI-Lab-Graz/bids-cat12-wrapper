...
def parse_participants(args):
    ...
    if args.covariates:
        ...
        if missing_cov:
            print(f"Note: Covariates {', '.join(missing_cov)} not in participants.tsv → will extract from CAT12 XML reports")
        present_cov = [col for col in covariate_cols if col in df.columns]
        if present_cov:
            print(f"Note: Using covariates {', '.join(present_cov)} from participants.tsv")
            ...
            if filepath:
                ...
                if file_smooth:
                    ...
                    if file_smooth != args.smoothing:
                        print(f"  ⚠ Warning: {subject} ses-{session} has {file_smooth}mm smoothing, expected {args.smoothing}mm")
                ...
                if covariate_cols:
                    for cov in covariate_cols:
                        ...
                        if cov in row.index and not pd.isna(row[cov]):
                            ...
                            xml_path = find_xml_for_subject(args.cat12_dir, subject, session)
                            if xml_path:
                                measure = _resolve_xml_covariate(cov)
                                val = extract_measure_from_xml(xml_path, measure)
                                if val is not None:
                                    print(f"  → Extracted {cov}={val} from XML for {subject} ses-{session}")
                        ...
                        try:
                            ...
                            print(f"Error: Covariate '{cov}' for subject {subject} session {session} is not numeric: {e}")
                            sys.exit(1)
    ...
    for cov in covariate_cols:
        ...
        if missing_entries:
            ...
            print(f"⚠ Missing {len(missing_entries)} value(s) for '{cov}' (examples: {sample})")
            print("  Provide the missing values in participants.tsv or CAT12 XML or rerun without that covariate.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--allow-missing-covariates', action='store_true',
